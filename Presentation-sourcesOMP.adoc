...
Dans cette partie, on focalise sur la manière de créer des parties pouvant s'exécuter en parallèle et des parties exécutées en un seul exemplaire, en précisant par quel thread.
...
. Au niveau du shell : la compilation au fromat parallèle ayant été réalklisée, on peut utiliser une variable d'environnement pour le préciser
. Au niveau du processus : au débt du code, on peut préciser le degré de parallélisme à utiliser tout le long de l'exécution de ce programme parallèle
