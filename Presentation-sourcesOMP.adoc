...
Le premier exemple a pour but de démarrer plusieurs threads en parallèle et de montrer que chacun a une identité propre, puisque chacun affiche un numéro différent.
...
On aurait pu utiliser une variable partagée et demander à un seul thread de l'affecter avec le nombre de threads. Malheuresement cela nous obligerait à synchroniser tous les threads.
...
De toute manière, tous les threads attendent que celui qui exécute la section ait fini.

L'exemple suivant permet de rapidement créer tous les threads en parallèle mais de définir séparément ce que chaque thread doit faire de sont coté.
...
* [*] Je sais séparer code parallèle et code à éxécuter par un seul thread
* [*] Je maitrise le degré de parallélisme
* [ ] Je maitrise le partage des données
...
Un équivalent serait de séparer cette section parallèle en deux sections successives mais on perdrait l'état et la valeur des données locales.
...
Le problème mis en évidence avec ce résultat incohérent est qu'une variable partagée (ici *b*) doit doit être protégée si on veut que plusieurs threads la modifient.
